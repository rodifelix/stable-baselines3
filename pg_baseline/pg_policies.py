...
import numpy as np
from pg_baseline import pg_hourglass, pg_light_multi_head_hourglass, pg_mask_net, pg_densenet
from torch.autograd import Variable
import torch.nn.functional as F
from stable_baselines3.common.policies import BasePolicy, register_policy

class LightHGNetwork(BasePolicy):
    """
    ...
    def __init__(
        ...
        preload_path: Optional[str],
        dueling: bool,
        noisy: bool,
    ):
        super(LightHGNetwork, self).__init__(
            observation_space,
            action_space,
        ...
        if self.preload_path is not None:
            state_dict = th.load(preload_path)
...
class PGDQNPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if self.net_class == "HG_Mask":
            ...
            self.net_args["preload_path"] = self.preload_mask_path
    ...
    def make_q_net(self) -> BasePolicy:
        if self.net_class == "HG" or self.net_class == "HG_Mask":
            ...
            return LightHGNetwork(**self.net_args).to(self.device)
